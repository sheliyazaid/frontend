...
export default function ParkingCrudPage({
  title,
  description,
  columns,
  api,
  ...
  useEffect(() => {
    slotsApi.list({ limit: 500 }).then((res) => setSlots(res.data.data));
    ...
    <div>
                    ...
                    <tr key={row._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          {extraActions?.(row, fetchData)}
                          {!hideEditDelete && api.update && (
                            <button
                              type="button"
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {renderForm({ register, errors, slots, flats, setValue, watch, editing })}
